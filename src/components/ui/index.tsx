...
export function Button({
  ...
  const base = 'inline-flex items-center justify-center font-bold rounded-xl transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed select-none';
  const variants: Record<string, string> = {
    primary: 'bg-black hover:bg-neutral-800 text-white shadow-sm active:bg-neutral-900',
    secondary: 'bg-neutral-100 hover:bg-neutral-200 text-neutral-900 font-semibold',
    outline: 'border border-neutral-300 hover:border-black hover:bg-neutral-50 text-neutral-900 bg-white',
    ghost: 'text-neutral-600 hover:text-black hover:bg-neutral-100',
    danger: 'bg-red-600 hover:bg-red-700 text-white',
    success: 'bg-emerald-600 hover:bg-emerald-700 text-white',
    glow: 'bg-black hover:bg-neutral-800 text-white font-bold',
  };
...
export function Card({ children, className = '', hover, padding = 'md', onClick }: CardProps) {
  ...
  return (
    <div
      className={`bg-white border border-neutral-200 rounded-2xl shadow-sm ${hover ? 'hover:border-neutral-400 hover:shadow-md cursor-pointer transition-all' : ''} ${pads[padding]} ${className}`}
      onClick={onClick}
...
export function Badge({ children, variant = 'default', size = 'sm', icon, className = '' }: BadgeProps) {
  const variants: Record<string, string> = {
    ...
    glow: 'bg-black text-white border-black font-bold',
  };
  const sizes: Record<string, string> = { sm: 'px-2.5 py-0.5 text-xs', md: 'px-3 py-1 text-sm' };
  return (
...
export function Modal({ open, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className={`relative bg-white border border-neutral-200 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full ${sizes[size]} max-h-[90vh] overflow-y-auto z-10`}>
        ...
        {title && (
          <div className="flex items-center justify-between p-6 border-b border-neutral-100">
            ...
            <button onClick={onClose} className="p-2 rounded-full text-neutral-400 hover:text-black hover:bg-neutral-100 transition-colors" aria-label="Close dialog">
              <X className="w-5 h-5" />
            </button>
          </div>
        )}
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input
          id={inputId}
          className={`w-full rounded-xl bg-neutral-100 hover:bg-neutral-200/70 focus:bg-white border border-transparent focus:border-black px-4 py-3.5 text-neutral-900 font-semibold placeholder:text-neutral-400 transition-all focus:outline-none ${icon ? 'pl-11' : ''} ${error ? 'border-red-500 focus:border-red-500' : ''} ${className}`}
          {...props}
        />
      </div>
...
export function Select({ label, options, className = '', id, ...props }: SelectProps) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <select
        id={selectId}
        className={`w-full rounded-xl bg-neutral-100 hover:bg-neutral-200/70 focus:bg-white border border-transparent focus:border-black px-4 py-3.5 text-neutral-900 font-semibold focus:outline-none transition-all cursor-pointer ${className}`}
        {...props}
      >
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer group select-none">
      <button
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-black ${checked ? 'bg-black' : 'bg-neutral-300'}`}
      >
...
export function ProgressBar({ value, max = 100, label, color, showValue = true, size = 'md' }: ProgressBarProps) {
  ...
  const barColor = color || (pct >= 80 ? 'bg-emerald-600' : pct >= 50 ? 'bg-amber-500' : 'bg-red-500');
    ...
    <div className="space-y-1">
      {(label || showValue) && (
        <div className="flex justify-between text-xs font-bold">
          ...
          {showValue && <span className="text-neutral-900">{Math.round(value)}%</span>}
        </div>
      )}
      <div className={`w-full ${heights[size]} bg-neutral-200 rounded-full overflow-hidden`}>
        <div className={`h-full ${barColor} rounded-full transition-all duration-300`} style={{ width: `${pct}%` }} role="progressbar" aria-valuenow={value} aria-valuemin={0} aria-valuemax={max} />
      </div>
    </div>
  );
}
...
export function RadialScore({ score, size = 'md', label, showLabel = true }: RadialScoreProps) {
  const dims: Record<string, { w: number; r: number; stroke: number; text: string }> = {
    sm: { w: 40, r: 16, stroke: 3.5, text: 'text-xs font-bold' },
    md: { w: 64, r: 26, stroke: 4.5, text: 'text-lg font-black' },
    lg: { w: 96, r: 40, stroke: 6, text: 'text-2xl font-black' },
  };
  const d = dims[size];
  const circ = 2 * Math.PI * d.r;
  const offset = circ - (Math.min(100, Math.max(0, score)) / 100) * circ;
  const strokeColor = score >= 80 ? '#06c167' : score >= 50 ? '#f38b00' : '#e11900';

  return (
    <div className="flex flex-col items-center gap-1">
      <div className="relative flex items-center justify-center">
        <svg width={d.w} height={d.w} className="transform -rotate-90">
          <circle cx={d.w / 2} cy={d.w / 2} r={d.r} fill="none" stroke="#eeeeee" strokeWidth={d.stroke} />
          <circle
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className={`${d.text} text-neutral-900 leading-none`}>
            {score}
          </span>
        </div>
      </div>
      {showLabel && label && <span className="text-xs font-bold text-neutral-600">{label}</span>}
    </div>
  );
}
...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      <div className="w-16 h-16 rounded-2xl bg-neutral-100 flex items-center justify-center text-neutral-800 mb-4">
        {icon}
      </div>
      <h3 className="text-xl font-bold text-neutral-900 mb-1">{title}</h3>
      {description && <p className="text-sm text-neutral-600 max-w-sm mb-6">{description}</p>}
      {action}
    </div>
  );
}
...
export function ToastItem({ type, message, onClose }: ToastItemProps) {
  ...
  return (
    <div className="flex items-center gap-3 bg-neutral-900 text-white rounded-xl px-4 py-3.5 min-w-[280px] max-w-md shadow-2xl z-50" role="alert">
      {icons[type]}
      <span className="flex-1 text-xs font-semibold">{message}</span>
      <button onClick={onClose} className="p-1 rounded-lg text-neutral-400 hover:text-white" aria-label="Dismiss">
        <X className="w-4 h-4" />
      </button>
    </div>
  );
    ...
    <div className="flex gap-1 bg-neutral-100 p-1 rounded-xl overflow-x-auto no-scrollbar" role="tablist">
      {tabs.map(tab => (
        <button
          ...
          className={`px-4 py-2 text-xs font-bold rounded-lg transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-black text-white shadow-sm' : 'text-neutral-600 hover:text-black'}`}
        >
          {tab.label}
          {tab.count !== undefined && tab.count > 0 && (
            <span className={`ml-1.5 px-1.5 py-0.5 text-[10px] rounded-full ${activeTab === tab.id ? 'bg-white/20 text-white' : 'bg-neutral-200 text-neutral-800'}`}>
              {tab.count}
            </span>
          )}
        </button>
...
export function MetricCard({ icon, label, value }: MetricCardProps) {
  return (
    <div className="bg-white rounded-2xl p-5 border border-neutral-200 shadow-sm">
      ...
      <div className="text-xs font-bold text-neutral-500 mt-0.5">{label}</div>
    </div>
  );
}
