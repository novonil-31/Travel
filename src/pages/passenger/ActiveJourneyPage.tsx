...
import {
  ...
  ShieldCheck, ArrowRight, Check, AlertTriangle,
...
export default function ActiveJourneyPage() {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-4 sm:py-6 space-y-4 font-sans">
      ...
      <div className="bg-black text-white p-3.5 sm:p-5 rounded-3xl shadow-lg space-y-2.5">
        {/* Top Controls Row */}
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="px-2 py-1 rounded-full bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-white text-[10px] sm:text-xs font-bold transition-colors flex items-center gap-1 cursor-pointer"
            ...
            <button
              ...
              className="px-2 py-1 rounded-full bg-red-500/20 hover:bg-red-500/30 text-red-300 hover:text-red-200 text-[10px] sm:text-xs font-bold transition-colors flex items-center gap-1 cursor-pointer"
            >
        ...
        {/* Telemetry Metrics Row: Distance, ETA, Speed */}
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="bg-neutral-800 text-emerald-400 font-extrabold px-2.5 py-0.5 rounded-full text-xs">
            {!hasReachedStartOrigin && liveDistToStartMeters > 40
          ...
          <span className="bg-neutral-800 text-neutral-300 font-bold px-2 py-0.5 rounded-full text-[10px]">
            ⚡ {hasReachedStartOrigin ? (isBus ? '28' : isTrain ? '75' : isFlight ? '540' : '32') : '4.5'} km/h
          </span>
        </div>
        ...
        <div className="pt-0.5 flex items-center justify-between gap-2">
          <div className="min-w-0 flex-1">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block">
              {!hasReachedStartOrigin && liveDistToStartMeters > 40 ? 'Start Location:' : 'Target Stop:'}
            </span>
            <p className="text-sm sm:text-base font-black text-white leading-snug mt-0.5 truncate">
              {!hasReachedStartOrigin && liveDistToStartMeters > 40 ? activeJourney?.originName : targetLocationName}
            </p>
            <p className="text-[11px] text-neutral-300 mt-0.5 font-medium truncate">
              {!hasReachedStartOrigin && liveDistToStartMeters > 40
                ? `Follow pedestrian path to boarding stand`
                : `${snappedResult.distanceFromStartMeters}m along road • ${routeProgressPercent}% done`}
            </p>
          </div>
          ...
          {!hasReachedStartOrigin && liveDistToStartMeters > 40 && (
            <button
              ...
              className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black shrink-0 flex items-center gap-1 shadow-md transition-all cursor-pointer active:scale-95"
            >
